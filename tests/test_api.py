...
# --- retryable (patrz coordinator._async_ask_ai_with_retry) --------------


def test_5xx_is_retryable():
    session = FakeSession(FakeResponse(503, text="server overloaded"))
    with pytest.raises(MeterReaderApiError) as exc_info:
        run(async_ask_ai(session, "gemini", "key", "gemini-2.5-flash", "prompt", b"jpeg"))
...
def test_auth_error_is_never_retryable():
    session = FakeSession(FakeResponse(401, payload={}))
    with pytest.raises(MeterReaderAuthError) as exc_info:
        run(async_ask_ai(session, "gemini", "bad-key", "gemini-2.5-flash", "prompt", b"jpeg"))
    assert exc_info.value.retryable is False
...
def test_connection_error_is_retryable():
    ...
    with pytest.raises(MeterReaderApiError) as exc_info:
        run(async_ask_ai(FailingSession(), "gemini", "key", "gemini-2.5-flash", "prompt", b"jpeg"))
    assert exc_info.value.retryable is True
