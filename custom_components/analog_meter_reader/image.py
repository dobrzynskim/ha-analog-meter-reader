...
def draw_timestamp(image: Image.Image, text: str) -> Image.Image:
    """Zwraca kopię zdjęcia ze znacznikiem czasu w prawym dolnym rogu - kiedy
    dokładnie zrobiono TO zdjęcie, żeby na encji `camera` (podgląd na
    dashboardzie) nie trzeba było zgadywać, czy widok jest świeży, czy
    sprzed kilku cykli. Tylko do zdjęcia w encji camera - nie dotyka
    przycięcia wysyłanego do AI (wywoływane osobno, na kopii)."""
    stamped = image.copy()
    ...
    box = (
        stamped.width - text_w - 2 * padding - margin,
        stamped.height - text_h - 2 * padding - margin,
        stamped.width - margin,
        stamped.height - margin,
    )
    draw.rectangle(box, fill=(0, 0, 0))
    draw.text((box[0] + padding, box[1] + padding), text, fill=(255, 255, 255), font=font)
    return stamped
