...
from .const import (
    ...
    CONF_FRACTION_DIGIT_COLOR,
    CONF_GEMINI_MODEL,
    CONF_MAX_STEP,
    CONF_PROMPT,
    CONF_QUIET_HOURS_END,
    CONF_QUIET_HOURS_START,
    CONF_WHOLE_DIGIT_COLOR,
    CONSECUTIVE_BAD_THRESHOLD,
    CROP_UPSCALE_FACTOR,
    DEFAULT_AI_PROVIDER,
    DEFAULT_FLIP_HORIZONTAL,
    DEFAULT_FRACTION_DIGIT_COLOR,
    DEFAULT_MAX_STEP,
    DEFAULT_MODEL_BY_PROVIDER,
    DEFAULT_WHOLE_DIGIT_COLOR,
    DOMAIN,
    STORAGE_VERSION,
)
from .image import crop_for_ocr, draw_timestamp, load_and_orient, to_jpeg_bytes
from .prompt import build_default_prompt
...
class MeterReaderCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    async def async_prime_from_storage(self) -> None:
        ...
        self.async_set_updated_data(
            {**(self.data or {}), "value": self._last_good, "consecutive_bad": self._consecutive_bad}
        )
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        ...
        # Liczone niezależnie od "force" - to "czy jesteśmy teraz w oknie
        # ciszy" samo w sobie (encja binary_sensor "w oknie ciszy"), nie
        # "czy AKURAT pominęliśmy cykl z jej powodu".
        quiet_active = is_within_quiet_hours(dt_util.now().time(), quiet_start, quiet_end)
        if not force and quiet_active:
        ...
        # Przycięcie do AI liczone z czystego "image" (bez znacznika czasu -
        # nie ma po co dokładać wizualnego szumu do tego, co i tak trafia do
        # promptu). Znacznik dorysowany dopiero na osobnej kopii, tylko do
        # zdjęcia w encji camera (podgląd na dashboardzie).
        ...
        stamped_image = await self.hass.async_add_executor_job(
            draw_timestamp, image, dt_util.now().strftime("%Y-%m-%d %H:%M:%S")
        )
        full_bytes = await self.hass.async_add_executor_job(to_jpeg_bytes, stamped_image)

        whole_color = self._config.get(CONF_WHOLE_DIGIT_COLOR) or DEFAULT_WHOLE_DIGIT_COLOR
        fraction_color = self._config.get(CONF_FRACTION_DIGIT_COLOR) or DEFAULT_FRACTION_DIGIT_COLOR
        prompt = self._config.get(CONF_PROMPT) or build_default_prompt(whole_color, fraction_color)
        ...
        try:
            text = await self._async_ask_ai_with_retry(
                provider, self._config[CONF_API_KEY], model, prompt, crop_bytes, base_url
            )
        except MeterReaderApiError as err:
            raise UpdateFailed(str(err)) from err
        ...
        if value is None:
            ...
            result["consecutive_bad"] = self._consecutive_bad
            return result
    ...
    async def _async_ask_ai_with_retry(
        self,
        provider: str,
        api_key: str,
        model: str,
        ...
        """Krótki retry z backoffem na przejściowe błędy (patrz
        api.MeterReaderApiError.retryable) - jeden dodatkowy strzał w tym
        samym cyklu, zamiast czekania na kolejny, pełny scan_interval.
        Błędy nie-przejściowe (zły klucz, model nie istnieje) lecą dalej
        od razu - ponawianie identycznego zapytania niczego by nie dało."""
        for attempt in range(AI_RETRY_ATTEMPTS):
            try:
                return await async_ask_ai(
                    self._session, provider, api_key, model, prompt, image_bytes, base_url=base_url
                )
            except MeterReaderApiError as err:
                if not err.retryable or attempt == AI_RETRY_ATTEMPTS - 1:
                    raise
                _LOGGER.warning(
                    "Błąd AI (próba %d/%d), ponawiam za %ds: %s",
                    attempt + 1, AI_RETRY_ATTEMPTS, AI_RETRY_BACKOFF_SECONDS, err,
                )
                await asyncio.sleep(AI_RETRY_BACKOFF_SECONDS)
