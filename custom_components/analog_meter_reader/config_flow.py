        ...
        schema = vol.Schema(
            {
                ...
                # Większość liczników wody/gazu ma czarne cyfry pełnych
                # jednostek i czerwone cyfry ułamka na osobnym bębenku - ale
                # nie wszystkie (patrz prompt.build_default_prompt). Wpływają
                # tylko na domyślny prompt, nie na własny (CONF_PROMPT) -
                # ten ma zawsze pierwszeństwo, jeśli jest ustawiony.
                vol.Optional(
                    CONF_WHOLE_DIGIT_COLOR,
                    default=options.get(CONF_WHOLE_DIGIT_COLOR, DEFAULT_WHOLE_DIGIT_COLOR),
                ): str,
                vol.Optional(
                    CONF_FRACTION_DIGIT_COLOR,
                    default=options.get(CONF_FRACTION_DIGIT_COLOR, DEFAULT_FRACTION_DIGIT_COLOR),
                ): str,
